...
- Runtime 按用户动态启停，scene 只用于 OpenCode 会话创建时的 workdir、流程配置、agents 和 skills 选择。
...
3. **实例层**：按用户运行 OpenCode Agent 工作负载，通过 NAS 挂载用户存储根路径。scene 不决定 Runtime 实例归属，只在 OpenCode 创建会话时用于拼接会话 workdir、加载对应流程配置、agents 和 skills，实现会话级资源与配置隔离。
...
scene 是会话级参数，不是 Runtime 实例级参数。Runtime 按用户动态启停；当用户通过 Agent API 创建 OpenCode 会话时，服务读取会话创建请求中的 `scene`，按规则拼接会话 workdir，并加载该 scene 对应的流程配置、agents 和 skills。
...
- scene 只用于会话 workdir、流程配置、agents 和 skills 选择。
...
NAS 用于挂载用户存储根路径和 OpenCode scene 配置。Runtime 创建时只挂载用户存储根路径；scene 在 OpenCode 创建会话时参与 workdir 与配置路径拼接，不参与 Runtime 实例创建。
...
| scene 配置 NAS 目录 | `{nas.path}/scenes/{scene}/opencode` |
| scene 配置容器内目录 | `{runtime.config}/{scene}/opencode` |
...
- scene 配置目录按 scene 隔离，作为 OpenCode 会话流程配置、agents 和 skills 的加载来源。
...
代理入口以 OpenCode Server API 为准。OpenCode 官方 Server 文档说明，`opencode serve` 暴露 OpenAPI 3.1 规范，规范页面为 Runtime 内部的 `/doc`。除会话创建请求需要处理 agent-control 的 `scene` 扩展参数外，本服务不自定义 OpenCode API 路径，只把 `/api/v1/runtime/agent` 前缀去掉后转发到当前用户 Runtime Service。
...
- `scene` 是 agent-control 的会话级扩展参数，用于选择 workdir、流程配置、agents 和 skills。
...
- 转换后的容器内 scene 配置目录为 `{runtime.config}/{scene}/opencode`。
...
| `runtime.port` | OpenCode Runtime 容器监听端口。 |
| `runtime.workdir` | 用户存储根路径在 Runtime 容器内的挂载路径。 |
| `runtime.config` | scene 配置根路径在 Runtime 容器内的挂载路径。 |
| `nas.path` | NAS 挂载基础路径。 |
