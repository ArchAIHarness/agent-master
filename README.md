...
- 保持已确认的 PVC + subPath 挂载逻辑：Runtime Deployment 禁止使用 `hostPath`；通过 `runtime.workspacePvcClaimName` 指定 PVC，并将 `{runtime.workspacePvcSubPathRoot}/{userId}/runtime -> /app`、`{runtime.workspacePvcSubPathRoot}/{userId}/global -> ~` 作为 subPath 挂载，OpenCode 默认路径自然匹配持久化。
...
`agent-master` 使用 `main` + `variants/*` 管理长期演进线，避免把长期产品形态误命名为短期 `feat/*` 或发布 `release/*` 分支。

| 分支 | 定位 | 演进边界 |
|---|---|---|
| `main` | 稳定主线 | 汇总已稳定、可作为默认基线的能力。 |
| `variants/headless-opencode` | Headless OpenCode Runtime | 只维护 OpenCode Agent API / SSE / PTY WebSocket，不承接浏览器 WebUI。 |
| `variants/aionui-webui` | AionUi WebUI Runtime | 面向 `agent-image-webui` / AionUi 形态演进，独立维护 AionUi 所需反代与配置。 |
| `variants/opensumi-agent-webui` | OpenSumi agent-webui Runtime | 面向 `agent-webui` / OpenSumi IDE 形态演进，独立维护 OpenSumi WebUI、AG-UI 与相关联调能力。 |

分支治理约束：

- 三个 `variants/*` 分支是长期变体分支，不作为一次性功能分支使用。
- 不同 WebUI 形态的实现、配置和文档优先在各自变体分支内演进，避免 AionUi 与 OpenSumi 路径、端口、协议假设相互污染。
- 从私有联调或实验分支合并回 `main` 前，必须明确目标变体是否已稳定，并补齐测试、文档和配置说明。
...
- 用户默认项目级 OpenCode 配置位于 `{runtime.workdir}/{userId}/runtime/.opencode`，通过 runtime subPath 挂载自然成为 `/app/.opencode`。
...
- Runtime Deployment 必须使用 `runtime.workspacePvcClaimName` 指向的 PVC，通过 subPath 分别挂载 `{runtime.workspacePvcSubPathRoot}/{userId}/runtime` → `/app` 和 `{runtime.workspacePvcSubPathRoot}/{userId}/global` → `~`。
- Runtime Deployment 禁止使用 `hostPath`；本地开发、测试和生产渲染逻辑都必须保持 PVC + subPath 形态。
...
### 7.9 OpenSumi agent-webui 代理接口

`variants/opensumi-agent-webui` 分支新增 OpenSumi / `agent-webui` 浏览器入口：
...
- 仅当 `runtime.agentWebuiPort` 已配置时注册 `/webui/*`；未配置时不暴露 WebUI 入口。
- `/webui/*` 基于 `x-user-id` 查找当前用户正在运行的 Runtime，不接受 `runtimeId` 作为路径参数。
- 转发目标为当前 Runtime Service 的 `runtime.agentWebuiPort`，默认 `3000`，不复用 OpenCode API 端口 `4096`。
- 配置 `runtime.agentWebuiPort` 时，Deployment 不覆盖镜像 `command/args`，由 `agent-webui` 镜像自己的 entrypoint/supervisord 同时启动 OpenCode 与 OpenSumi WebUI。
- 与 AionUi 变体不同，OpenSumi `agent-webui` 容器内路由本身以 `/webui` 为前缀，因此代理到上游时保留 `/webui` 前缀：`/webui/adapter-info` → `/webui/adapter-info`。
- `Authorization`、`x-user-id`、`Host`、`Proxy-Authorization` 和 hop-by-hop Header 不透传给 `agent-webui`；浏览器 `Cookie` 可保留。
- 当前本入口覆盖 HTTP 与 SSE；OpenSumi WebSocket 路径需经本地联调确认后再独立纳入，不与 `/agent/ws/*` 混用。
...
`variants/opensumi-agent-webui` 分支的 Runtime Service 暴露两个端口：

| Service Port 名称 | 端口 | 目标 | 说明 |
|---|---:|---|---|
| `opencode-http` | `4096` | OpenCode Server | `/agent/*` HTTP/SSE 与 `/agent/ws/*` PTY WebSocket 使用。 |
| `webui-http` | `3000` | OpenSumi agent-webui | `/webui/*` HTTP/SSE 使用；端口名保持 15 字符以内以符合 Kubernetes 约束。 |
...
| `runtime.image` | OpenCode Agent 镜像；`variants/opensumi-agent-webui` 默认使用 `archai/agent-webui:m1`。 |
| `runtime.ttl` | Agent 租约有效期，连接存在时续约到 1 小时。 |
| `runtime.port` | OpenCode Agent 容器监听端口，当前与 OpenCode 运行时约定为 `4096`，Agent 需监听 `0.0.0.0` 以便 Service 访问。 |
| `runtime.agentWebuiPort` | 可选 OpenSumi agent-webui HTTP 端口；`variants/opensumi-agent-webui` 使用 `3000`，未配置时不暴露 `/webui/*`。 |
| `runtime.agentWebuiPathPrefix` | 可选 OpenSumi agent-webui 浏览器侧路径前缀，默认 `/webui`；代理到上游时保留该前缀以匹配 agent-webui 容器内路由。 |
| `runtime.workdir` | `agent-master` 容器内用于初始化用户目录的 NAS 工作目录根路径，必须与 Runtime PVC 指向同一套存储。 |
| `runtime.workspacePvcClaimName` | Runtime Deployment 使用的 PVC 名称；Runtime Pod 禁止使用 `hostPath`。 |
| `runtime.workspacePvcSubPathRoot` | PVC 内用户目录前缀；Kubernetes 启动 Agent 时通过两个 subPath 分别挂载：<br>- `{runtime.workspacePvcSubPathRoot}/{userId}/runtime` → 容器 `/app`（用户项目工作目录，包含 `AGENTS.md` + `.opencode/`）<br>- `{runtime.workspacePvcSubPathRoot}/{userId}/global` → 容器 `~`（OpenCode 全局配置/data/cache 对应用户主目录 `.config/`、`.local/share/`、`.cache/`，正好匹配 OpenCode 默认查找路径，不需要修改代码） |
...
Runtime Deployment 必须使用 `runtime.workspacePvcClaimName` 指定的 PVC，禁止渲染 `hostPath`，并通过两个 subPath 分别挂载：

| PVC 内 subPath | 容器内路径 | 挂载方式 | 说明 |
|---|---|---|---|
...
- Runtime Deployment volumes 只能出现 `persistentVolumeClaim.claimName`，不能出现 `hostPath`。
- PVC 内按 `{runtime.workspacePvcSubPathRoot}/{userId}` 分两个 subPath 挂载，结构清晰不冲突。
- OpenCode 默认查找 `~/.config/opencode` 等路径，正好对应用户 `global/` subPath 下的路径，完美匹配。
