...
- 保持已确认的 Runtime PVC 挂载逻辑：`runtime.workdir` 仅用于 agent-master 侧初始化物理目录；Runtime Deployment 使用 `runtime.workspacePvcClaimName` 指定的 PVC，并通过 `{runtime.workspacePvcSubPathRoot}/{userId}/runtime -> /app`、`{runtime.workspacePvcSubPathRoot}/{userId}/global -> /root` 两个 subPath 持久化，OpenCode 默认路径自然匹配。
...
- Runtime Deployment 使用 `runtime.workspacePvcClaimName` 指定的 PVC，通过两个 subPath 分别挂载：
  - `{runtime.workspacePvcSubPathRoot}/{userId}/runtime` → `/app`（用户项目工作目录，`AGENTS.md` + `.opencode/` 自然成为 `/app/AGENTS.md` + `/app/.opencode`）
  - `{runtime.workspacePvcSubPathRoot}/{userId}/global` → `/root` / `~`（OpenCode 默认查找路径正好匹配全局配置/data/cache）
...
- Runtime Deployment 禁止使用 `hostPath`；整个用户存储必须来自 `runtime.workspacePvcClaimName` 指定的 PVC。
- `runtime.workspacePvcSubPathRoot` 是 PVC 内用户目录根，例如 `agent-master/users`；Deployment 通过 `agent-master/users/{userId}/runtime` → `/app`、`agent-master/users/{userId}/global` → `/root` 两个 subPath 挂载。
...
| `runtime.workdir` | `agent-master` 初始化用户目录时使用的 NAS 工作目录根路径。Deployment 不得把该路径渲染为 `hostPath`。 |
| `runtime.workspacePvcClaimName` | Runtime Deployment 使用的用户存储 PVC 名称，必填；Deployment volumes 只能生成指向该 PVC 的 `persistentVolumeClaim`，禁止 `hostPath`。 |
| `runtime.workspacePvcSubPathRoot` | PVC 内用户目录根，必填；Deployment 通过 `{runtime.workspacePvcSubPathRoot}/{userId}/runtime` → `/app`、`{runtime.workspacePvcSubPathRoot}/{userId}/global` → `/root` 两个 subPath 挂载。 |
...
Runtime Deployment 禁止使用 `hostPath`。整个用户目录来自 `runtime.workspacePvcClaimName` 指定的 PVC，并通过两个 subPath 分别挂载：

| 源路径 (NAS) | 容器内路径 | 挂载方式 | 说明 |
|---|---|---|---|
| `{runtime.workspacePvcSubPathRoot}/{userId}/runtime` | `/app` | subPath | 用户**项目工作目录**；覆盖镜像内 `/app` 是预期行为；`AGENTS.md` 与 `.opencode/` 作为用户默认项目配置自然生效。 |
| `{runtime.workspacePvcSubPathRoot}/{userId}/global` | `/root` | subPath | 用户**全局数据目录**，完整对应用户主目录结构：<br>- `global/.config/opencode` → `/root/.config/opencode` <br>- `global/.local/share/opencode` → `/root/.local/share/opencode` <br>- `global/.cache/opencode` → `/root/.cache/opencode` <br>正好匹配 OpenCode 默认查找路径，**不需要修改 OpenCode 任何代码**。

渲染约束：

- `deploy.yaml` 中的用户路径必须来自 `x-user-id` 和服务端配置拼接，不接受客户端传入任意挂载路径。
- Deployment volumes 只允许包含 `persistentVolumeClaim`，禁止 `hostPath`。
- 整个用户目录来自同一个 PVC，分两个 subPath 挂载，结构清晰不冲突。
- OpenCode 默认查找 `~/.config/opencode` 等路径，正好对应 PVC `global/` 下的路径，完美匹配。
