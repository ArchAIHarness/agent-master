...
> Agent 控制面服务：负责单 Kubernetes 集群下 OpenCode Agent 工作负载的调度启停，并代理转发对应 Agent API 请求。

## 1. 定位与目标

`agent-master` 是一个 Agent 控制面服务，负责根据用户或上游平台请求，在 Kubernetes 集群的目标 Namespace 中创建、查询、关闭 OpenCode Agent 工作负载，并代理转发对应 Agent API 请求。
...
- 服务在 Kubernetes 集群的目标 Namespace 中创建、查询、关闭对应 Agent 工作负载。
...
`agent-master` 对外提供 Agent 管理 API 和 Agent API 代理入口，对内通过 Redis 维护实例状态，通过 Kubernetes 适配器调度和管理 OpenCode Agent 工作负载。
...
2. **调度控制层**：负责 Agent 实例状态、Redis 映射、创建/查询/关闭幂等、租约续约、TTL 回收，以及 Deployment（1 副本）+ Service 的创建、查询和删除。
...
- 保持已确认的 NAS 挂载逻辑：Runtime Deployment 禁止 hostPath，必须使用单 PVC `{runtime.workspacePvcClaimName}` + subPath；`{runtime.workspacePvcSubPathRoot}/{userId}/runtime -> /app`，`{runtime.workspacePvcSubPathRoot}/{userId}/global -> ~`，OpenCode 默认路径自然匹配持久化。
- 保持已确认的 Agent API 通用透明代理逻辑：去掉 `/agent` 前缀，保留 OpenCode 官方路径、查询参数、HTTP 方法和请求体。
- OpenCode `directory` 是官方 query 参数，由调用方显式传入。
- 当单集群 Agent Deployment 数量增长导致 Kubernetes Server API 压力上升时，通过扩展 Namespace 容量分摊负载。
- Namespace 扩展只影响 Agent 调度落点，不改变用户侧 API、Agent 容器内目录结构和透明代理约定。
...
| Kubernetes 集成 | Kubernetes Client Adapter | 通过适配器封装 Deployment（1 副本）+ Service 创建、查询、关闭和代理目标解析。 |
...
- Runtime Deployment 使用单个 PVC `{runtime.workspacePvcClaimName}`，通过两个 subPath 分别挂载：
...
- 用户默认项目规则位于 `{runtime.workdir}/{userId}/runtime/AGENTS.md`，Runtime Deployment 通过 PVC subPath `{runtime.workspacePvcSubPathRoot}/{userId}/runtime -> /app` 挂载后自然成为 `/app/AGENTS.md`。
- 用户默认项目级 OpenCode 配置位于 `{runtime.workdir}/{userId}/runtime/.opencode`，通过同一个 `/app` subPath 挂载自然成为 `/app/.opencode`。
- OpenCode 用户级配置持久化到 `{runtime.workdir}/{userId}/global/.config/opencode`，Runtime Deployment 通过 PVC subPath `{runtime.workspacePvcSubPathRoot}/{userId}/global -> ~` 挂载后自然成为 `~/.config/opencode`。
- OpenCode 用户级数据持久化到 `{runtime.workdir}/{userId}/global/.local/share/opencode`，通过 `global -> ~` subPath 挂载自然成为 `~/.local/share/opencode`，包含 `auth.json`、会话记录、索引、缓存等。
- OpenCode 动态下载的 provider 包和插件缓存持久化到 `{runtime.workdir}/{userId}/global/.cache/opencode`，通过 `global -> ~` subPath 挂载自然成为 `~/.cache/opencode`，避免每次重启重新下载。
...
- `{runtime.workdir}/{userId}` 是 master 初始化用户目录时使用的 NAS 物理路径根。
- Runtime Deployment 使用 PVC `{runtime.workspacePvcClaimName}`，并以 `{runtime.workspacePvcSubPathRoot}/{userId}` 作为 PVC 内用户目录根。
- 用户根下分两个顶层子目录，分别通过 PVC subPath 挂载到容器不同位置：
...
- Runtime Deployment 禁止使用 hostPath；本地、测试和生产环境都必须使用 PVC + subPath，避免依赖节点本地路径。
- 整个用户目录通过单个 PVC `{runtime.workspacePvcClaimName}` 承载，通过 subPath `{runtime.workspacePvcSubPathRoot}/{userId}/runtime` → `/app` 和 `{runtime.workspacePvcSubPathRoot}/{userId}/global` → `~` 分别挂载。
...
`agent-master` 在配置指定的单个 Kubernetes 集群与 Namespace 中创建 Agent Deployment（1 副本）+ Service；Pod 在节点上的最终落点由 Kubernetes Scheduler 决定。

- 当前版本不在创建 Agent 前主动查询 Kubernetes 资源状态（Node、ResourceQuota、LimitRange、现有 Agent 分布、Event 等），不做集群间调度均衡。
- 目标集群与 Namespace 由 `config.yaml` 的 `kubernetes.cluster` 和 `kubernetes.namespace` 固定提供。
- 如需影响节点落点，可在 Deployment 规格中配置 nodeSelector、affinity、tolerations 或 priorityClass，但必须由上游策略或服务配置显式提供。
- 如需容量保护、资源预检或多集群均衡，由上游平台控制面承担，不在本服务实现。
...
- 跨集群访问（如需）：通过受控 Secret、挂载文件或外部配置系统注入 kubeconfig / Token / 证书引用。
...
| `runtime.port` | OpenCode Agent 容器监听端口，当前与 `agent-runtime` 镜像约定为 `4096`，Agent 需监听 `0.0.0.0` 以便 Service 访问。 |
 | `runtime.workdir` | 用户 NAS 工作目录根路径，供 master 初始化用户目录使用；不作为 Runtime Deployment hostPath。 |
| `runtime.workspacePvcClaimName` | Runtime Deployment 挂载用户存储使用的 PVC 名称，必填；Deployment 只允许使用该 PVC，不允许 hostPath。 |
| `runtime.workspacePvcSubPathRoot` | Runtime Deployment PVC 内的用户目录 subPath 根，例如 `agent-master/users`；最终挂载为 `{runtime.workspacePvcSubPathRoot}/{userId}/runtime` → `/app`、`{runtime.workspacePvcSubPathRoot}/{userId}/global` → `~`。 |
| `kubernetes.cluster` | Kubernetes 集群逻辑名。 |
| `kubernetes.namespace` | Agent Namespace。 |
...
Runtime Deployment 禁止 hostPath；整个用户目录通过 PVC `{runtime.workspacePvcClaimName}` 承载，并通过两个 subPath 分别挂载：

| 源路径 (NAS) | 容器内路径 | 挂载方式 | 说明 |
|---|---|---|---|
| `{runtime.workspacePvcSubPathRoot}/{userId}/runtime` | `/app` | PVC subPath | 用户**项目工作目录**；覆盖镜像内 `/app` 是预期行为；`AGENTS.md` 与 `.opencode/` 作为用户默认项目配置自然生效。 |
| `{runtime.workspacePvcSubPathRoot}/{userId}/global` | `~` | PVC subPath | 用户**全局数据目录**，完整对应用户主目录结构：<br>- `global/.config/opencode` → `~/.config/opencode` <br>- `global/.local/share/opencode` → `~/.local/share/opencode` <br>- `global/.cache/opencode` → `~/.cache/opencode` <br>正好匹配 OpenCode 默认查找路径，**不需要修改 OpenCode 任何代码**。

渲染约束：

- `deploy.yaml` 中的用户路径必须来自 `x-user-id` 和服务端配置拼接，不接受客户端传入任意挂载路径。
- Runtime Deployment 只允许渲染一个 `persistentVolumeClaim` volume，PVC 名称来自 `runtime.workspacePvcClaimName`；禁止渲染 hostPath。
- 整个用户目录位于 PVC 的 `{runtime.workspacePvcSubPathRoot}/{userId}` 下，分两个 subPath 挂载，结构清晰不冲突。
...
`agent-master` 与 Agent 工作负载部署在**两个独立 namespace**：master 自身在 `{masterNamespace}`（本地 `agent-master`），Agent 工作负载在 `{runtimeNamespace}`（本地 `agent-runtime`，对应 `config.yaml` 的 `kubernetes.namespace`）。master 通过**严格跨 namespace RBAC** 授权调度，并通过 Kubernetes Service DNS 代理访问 Agent。

> 当前版本已收敛为**单集群、无资源预检**：不读取多集群配置，不查询 Node / ResourceQuota / LimitRange / Event；`config.yaml` 只保留单集群 `kubernetes.cluster` / `kubernetes.namespace`。

### 10.1 部署清单结构

清单基于 Kustomize 分层，存储统一走 **NFS PV/PVC**：

```text
deploy/
├── k8s/
│   ├── base/                      # 与环境无关
│   │   ├── namespaces.yaml         # agent-master / agent-runtime 两个 namespace
│   │   ├── master/rbac.yaml        # SA + 跨 ns Role/RoleBinding（最小权限）
│   │   ├── master/configmap.yaml   # 单集群 config.yaml
│   │   ├── master/deployment.yaml  # master Deployment + Service
│   │   └── kustomization.yaml
│   └── local/                     # docker-desktop overlay
│       ├── nfs-pv-master.yaml      # master 侧 NFS PV + PVC（/nas）
│       ├── nfs-pv-runtime.yaml     # runtime 侧 NFS PV + PVC（Agent subPath 挂载）
│       ├── redis.yaml              # 本地 Redis
│       └── kustomization.yaml
└── local/
    ├── build-image.sh             # bun test/typecheck + docker build agent-master:local
    ├── deploy.sh                  # kubectl apply -k deploy/k8s/local + 等待就绪
    └── smoke-test.sh              # 端到端：调度 + 代理 + 回收
```

### 10.2 本地 docker-desktop 部署

本地存储由一个 **docker 容器跑 NFS server** 提供（不在集群内跑）。docker-desktop 的 kubelet 运行在 LinuxKit VM 上，NFS 容器使用 host 网络直接绑定 VM 的 `2049` 端口，因此 PV 的 `nfs.server` 使用 `127.0.0.1`（节点视角的本地地址，稳定可达）：

```bash
# 0. 启动 NFS server（docker host 网络，export 映射到容器运行 uid 1000）
docker run -itd --name agent-master-nfs --privileged --network host \
  -v agent-master-nfs:/nfsshare -v /lib/modules:/lib/modules:ro \
  -e NFS_EXPORT_0="/nfsshare *(rw,no_subtree_check,all_squash,anonuid=1000,anongid=1000,fsid=0)" \
  -e NFS_VERSION=4.2 -e NFS_DISABLE_VERSION_3=1 \
  erichough/nfs-server:2.2.1

# 1. 构建镜像到 docker-desktop 共享 daemon（无需 registry）
bash deploy/local/build-image.sh

# 2. 部署整套本地栈（namespaces / RBAC / NFS PV-PVC / Redis / agent-master）
bash deploy/local/deploy.sh

# 3. 端到端验证：创建 Agent（调度）+ /agent/* 代理 + 删除回收
bash deploy/local/smoke-test.sh
```

要点：

- PV `mountOptions` 使用 `nfsvers=4.2`、`sec=sys`，`nfs.server: 127.0.0.1`、`nfs.path: /`。
- export 使用 `all_squash,anonuid=1000,anongid=1000`，与 master/Agent 容器运行 uid（`bun`，1000）对齐，保证容器可读写。
- `runtime.image` 默认 `agent-image:latest`，本地需先准备好该镜像（docker-desktop 共享镜像，`imagePullPolicy: IfNotPresent`）。
- 生产环境替换为真实 NFS export，并把 `nfs.server` / `nfs.path` 指向真实地址。

### 10.3 跨 namespace RBAC

master 在 `{masterNamespace}` 持有 ServiceAccount；Role 与 RoleBinding 建在 `{runtimeNamespace}`，subject 指向 master 的 ServiceAccount。仅使用 namespace 级 Role，不使用 ClusterRole；只授予调度真正需要的最小权限：

- `apps/deployments`：`get,list,watch,create,patch,update,delete`
- `services`：`get,list,watch,create,patch,update,delete`
- `pods`：`get,list,watch`（仅用于读取 readiness）

### 10.4 从 0 到 1 部署顺序

1. 选择 `{masterNamespace}` 和 `{runtimeNamespace}` 并创建。
2. 在 `{runtimeNamespace}` 准备 Redis（可外部）和 NFS PV/PVC。
3. 构建并准备 `agent-runtime`（Agent）镜像。
4. 本地执行 `bun test`、`bun run typecheck`。
5. 构建并推送 `agent-master` 镜像。
6. 在 `{masterNamespace}` 创建 ConfigMap、Secret、Deployment、Service。
7. 创建跨 namespace RBAC（Role/RoleBinding 在 `{runtimeNamespace}`）。
8. 验证 `GET /health`、`POST /runtime`、`/agent/*` 代理。

### 10.5 部署后验证与发布前检查

```bash
kubectl -n {masterNamespace} port-forward svc/agent-master 3002:3000
curl http://127.0.0.1:3002/health
curl -X POST http://127.0.0.1:3002/runtime -H 'x-user-id: user-a'
kubectl -n {runtimeNamespace} get deploy,svc,pod -l userId=user-a
curl http://127.0.0.1:3002/agent/project/current -H 'x-user-id: user-a'
```
...
- `POST /runtime` 能在 `{runtimeNamespace}` 创建 Agent Deployment（1 副本）+ Service。
- `/agent/*` 能代理到 Agent Service。
- ConfigMap / Secret 不含真实 Token、Cookie、密码、证书、私钥。
- 上游网关注入可信 `x-user-id`；`Authorization`、`proxy-authorization` 和 hop-by-hop Header 不透传到 Agent。
