...
- Agent 按用户动态启停，Kubernetes 按约定挂载用户工作目录和预设场景配置。
...
3. **实例层**：按用户运行 OpenCode Agent 工作负载，通过 Kubernetes 挂载用户工作目录和预设场景配置。Agent 实例按用户归属创建和复用；scene 是 `agent-master` 的会话级扩展参数，用于拼接 OpenCode 会话目录，并在代理转发时转换为 OpenCode 官方 `POST /session?directory=/app/{scene}`。
...
- 保持已确认的 NAS 挂载逻辑：`{runtime.workdir}/{userId} -> /app`，OpenCode 用户级目录持久化到用户目录下的 `.runtime/opencode/`，预设 scene 配置经 `/scene-config/{scene}` 只读源目录复制到用户侧 `/app/{scene}`。
...
- 多集群扩展只影响 Agent 调度落点，不改变用户侧 API、Agent 容器内目录结构和 scene 挂载约定。
...
- Agent 创建或重启前，初始化流程必须确保用户目录 `{runtime.workdir}/{userId}`、用户默认 `AGENTS.md`、用户默认 `.opencode/`、OpenCode 用户级持久化目录 `.runtime/opencode/config` 与 `.runtime/opencode/share`、所有已声明 scene 的用户工作目录 `{runtime.workdir}/{userId}/{scene}/` 均已存在。
- Agent 创建或重启前，初始化流程必须确保所有已声明 scene 的预设配置源路径 `{runtime.scenes.<scene>}/AGENTS.md` 与 `{runtime.scenes.<scene>}/.opencode/` 均已存在。
...
- 根据 `runtime.scenes` 声明的全部预设 scene，将每个 `{runtime.scenes.<scene>}` 作为只读源目录挂载到 `/scene-config/{scene}`，再由 initContainer 复制到用户侧 `/app/{scene}`。
...
`agent-master` 约定 Kubernetes 挂载关系，并在代理 OpenCode 会话创建请求时处理 `scene` 扩展参数。Agent 启动时，用户工作目录和预设场景配置已经挂载到容器内；创建会话时传入的 `scene` 用于拼接容器内会话目录 `/app/{scene}`，并转换为 OpenCode 官方 `POST /session?directory=/app/{scene}`。
...
sequenceDiagram
    ...
    A->>S: 去掉 /agent 前缀并转发为 POST /session?directory=/app/{scene}
...
- `scene` 是 `agent-master` 的会话级扩展参数，不是 Agent 实例归属参数。
...
- `scene` 不参与 Agent Redis Key、Deployment、Service、镜像选择或调度均衡。
...
- `runtime.scenes` 约定的场景配置在 Agent 容器启动时通过 `/scene-config/{scene}` 只读源目录提供，再由 initContainer 复制到用户侧 `/app/{scene}/AGENTS.md` 和 `/app/{scene}/.opencode/`。
- 如果 Kubernetes ConfigMap、Secret、volumeMount 或 OpenCode 用户级运行配置发生更新，需要重启 Agent 容器 / Pod，让 OpenCode 进程重新启动后加载新配置。
- `agent-master` 只负责挂载路径约定、`scene -> directory` 转换和代理转发；具体如何在指定 directory 下应用项目规则，由 `agent-runtime` 的启动流程和 OpenCode 项目级配置约定实现。
...
NAS 用于承载三类目录：用户 Agent 工作目录、用户默认项目配置、预设 scene 配置。Agent 实例按用户创建和复用；Kubernetes 启动 Agent 时，按服务配置拼接源路径，并挂载到 Agent 容器内固定路径。
...
│       └── {scene}/
│           ├── AGENTS.md      # initContainer 从 scenes/{scene}/AGENTS.md 复制
│           ├── .opencode/     # initContainer 从 scenes/{scene}/.opencode/ 复制
│           └── ... 用户场景工作文件与运行产物，自由组织
└── scenes/
    ├── coding/
    │   ├── AGENTS.md
...
| `{runtime.scenes.<scene>}` | `/scene-config/{scene}` | 预设 scene 配置源目录，只读挂载给 initContainer；initContainer 将 `AGENTS.md` 和 `.opencode/` 复制到用户侧 `/app/{scene}`。 |
...
- Agent 创建或重启前，必须先初始化所有挂载依赖路径：`{runtime.workdir}/{userId}`、`{runtime.workdir}/{userId}/AGENTS.md`、`{runtime.workdir}/{userId}/.opencode/`、`{runtime.workdir}/{userId}/.runtime/opencode/config`、`{runtime.workdir}/{userId}/.runtime/opencode/share`、所有已声明 scene 的用户工作目录 `{runtime.workdir}/{userId}/{scene}/`，以及所有已声明 scene 的预设配置源路径 `{runtime.scenes.<scene>}/AGENTS.md` 与 `{runtime.scenes.<scene>}/.opencode/`。
- Kubernetes 启动 Agent 时，必须把 `runtime.scenes` 中声明的每个 scene 源目录只读挂载到 `/scene-config/{scene}`，由 initContainer 复制 `AGENTS.md` 和 `.opencode/` 到用户侧 `/app/{scene}`。
- `runtime.scenes` 的容器内源路径固定为 `/scene-config/{scene}`；复制目标固定为 `/app/{scene}/AGENTS.md` 和 `/app/{scene}/.opencode`，用户 scene 工作目录 `/app/{scene}` 保持承载工作文件和运行产物。
- `/app/{scene}/AGENTS.md` 和 `/app/{scene}/.opencode` 是 initContainer 从 `/scene-config/{scene}` 复制得到的用户侧文件，不是运行容器的直接只读挂载；避免遮蔽用户 scene 工作目录中的业务文件和运行产物。
...
- 用户默认项目配置、OpenCode 用户级运行配置和预设 scene 配置都是 Agent 启动时准备、启动时加载的配置，不是创建会话时动态加载的配置；配置更新后需要重启 Agent 容器 / Pod 才能重新加载。
- 新增 `runtime.scenes` 时，同样必须先创建用户侧 scene 工作目录 `{runtime.workdir}/{userId}/{scene}/` 和预设配置源路径 `{runtime.scenes.<scene>}/AGENTS.md`、`{runtime.scenes.<scene>}/.opencode/`；后续新建或重启的 Agent 按最新配置生成 source volume 并在 initContainer 中复制。
...
- Agent 不存在时返回 404。
- 重启不改变 Agent 归属、NAS 挂载约定、Service 代理入口或 scene 处理规则。
...
代理入口以 Agent 实际暴露的 OpenCode Server API 为准。`agent-runtime` 镜像使用 `opencode web --port 4096 --hostname 0.0.0.0` 启动，OpenAPI 3.1 规范页面为 Agent 内部的 `/doc`。Agent API 代理默认是通用透明代理：服务基于用户归属找到当前 Agent Service，去掉 `/agent` 前缀后保留 HTTP 方法、后续路径、查询参数和请求体转发。当前唯一 API 语义特例是 `POST /agent/session`：本服务将请求体扩展参数 `scene` 转换为 OpenCode 官方 `directory=/app/{scene}` query 参数，并从转发请求体中移除 `scene`。
...
| 创建会话 | `POST /agent/session` | `POST /session?directory=/app/{scene}` | `scene` 由代理层转换为 OpenCode 官方 `directory` query 参数，OpenCode 请求体不透传 `scene`。 |
...
| OpenCode directory 事件 | `GET /agent/event?directory=/app/{scene}` | `GET /event?directory=/app/{scene}` | 订阅指定 directory / workspace 相关 SSE 事件。 |
| OpenAPI 文档 | `GET /agent/doc` | `GET /doc` | OpenCode Server OpenAPI 文档。 |

会话代理规则：

- `POST /agent/session` 是当前唯一 API 语义特例；其它 OpenCode API 即使包含官方 `directory` query 参数，也按通用代理透传。
...
- 当代理 OpenCode 会话创建请求时，agent-master 根据 `scene` 拼接容器内目录 `/app/{scene}`，并转发为 `POST /session?directory=/app/{scene}`。
- `scene` 字段不透传给 OpenCode；转发请求体应移除 `scene`，只保留 OpenCode 官方支持的字段。
- `scene` 不触发配置动态加载，不改变 Agent 实例归属，不影响 Redis Key、Deployment、Service、镜像选择或调度均衡。
...
- 用户默认项目级 OpenCode 配置位于 `{runtime.workdir}/{userId}/.opencode`，通过根挂载自然成为 `/app/.opencode`。
- OpenCode 用户级运行配置持久化到 `{runtime.workdir}/{userId}/.runtime/opencode/config` 和 `{runtime.workdir}/{userId}/.runtime/opencode/share`，分别挂载到 `/root/.config/opencode` 与 `/root/.local/share/opencode`。
- `runtime.scenes` 中声明的所有场景配置都必须在 Agent 启动时通过 `/scene-config/{scene}` 源目录只读挂载提供，并由 initContainer 复制到用户侧 `/app/{scene}/AGENTS.md` 和 `/app/{scene}/.opencode/`；场景 `.opencode/` 可包含 `agents/`、`skills/`、`tools/` 等约束材料，不配置 `plugins/`、`commands/`、`modes/`。
- 如果 Kubernetes ConfigMap、Secret、volumeMount 或用户级 OpenCode 配置发生更新，需要重启 Agent 容器 / Pod，让 OpenCode 进程重新启动后加载新配置。

代理规则：

- 支持普通 HTTP 请求代理。
- 支持 OpenCode 原生 SSE 长连接代理；OpenCode 原生 SSE 仍走 `/agent/*` 通用代理入口，平台控制面事件使用独立的 `GET /runtime/events`；`/event` 可携带 `directory=/app/{scene}` 订阅指定 directory 事件，`/global/event` 用于 OpenCode Agent 全局事件。
...
Kubernetes 资源设计只围绕 Agent 运行单元展开，承接用户目录和预设 scene 配置的挂载约定，不承接完整平台控制面、审计中心、业务 scene 编排或运行时动态配置切换职责。
...
`deploy.yaml` 是 Agent 启动清单，必须体现 `runtime.workdir`、`runtime.scenes`、容器端口、镜像、环境变量、volume、volumeMount、Labels / Selector 和安全约束。
...
| `runtime.scenes` | 预设场景配置映射；每个条目的值是场景配置根目录，目录内包含 `AGENTS.md` 和 `.opencode/`；Kubernetes 启动 Agent 时挂载全部已声明 scene 到 `/scene-config/{scene}`，由 initContainer 复制到用户侧 `/app/{scene}`，用户场景工作目录 `/app/{scene}` 保持承载工作文件和运行产物。 |
...
| `{runtime.scenes.<scene>}` | `/scene-config/{scene}` | 预设 scene 配置源目录；initContainer 从中复制 `AGENTS.md` 与 `.opencode/` 到用户侧 `/app/{scene}`。 |
...
- `runtime.scenes` 中声明的所有预设 scene 都渲染为 `/scene-config/{scene}` 只读 source volume，供 initContainer 复制到用户侧 `/app/{scene}/AGENTS.md` 和 `/app/{scene}/.opencode/`。
...
- `runtime.scenes.<scene>` 源目录包含 `AGENTS.md` 和 `.opencode/`；渲染时将整个源目录挂载到 `/scene-config/{scene}`，再由 initContainer 复制到 `/app/{scene}`。
- 仅为 `runtime.scenes` 中声明的 scene 生成 source volume 和复制命令。
- 使用 source volume 或复制文件时，源文件、源目录和目标父目录必须由初始化流程预先创建，避免 Kubernetes 因挂载目标不存在或复制失败而启动失败。
- 新增 `runtime.scenes` 时，必须先创建用户侧 scene 工作目录和预设配置源路径；后续新建或重启的 Agent 按最新配置生成 volumeMount。
