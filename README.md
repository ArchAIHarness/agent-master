...
- Agent 按用户动态启停，Kubernetes 按约定挂载用户工作目录和Agent 预设配置包。
...
3. **实例层**：按用户运行 OpenCode Agent 工作负载，通过 Kubernetes 挂载用户工作目录和预设配置。Agent 实例按用户归属创建和复用；OpenCode `directory` 是官方接口 query 参数，由调用方显式传入并经 `/agent/*` 透明透传，`agent-master` 不做 `agentPreset -> directory` 转换。
...
- 保持已确认的 Agent API 通用透明代理逻辑：去掉 `/agent` 前缀，保留 OpenCode 官方路径、查询参数、HTTP 方法和请求体；`POST /agent/session` 不做 `agentPreset -> directory` 转换。
- OpenCode `directory` 是官方 query 参数，由调用方显式传入；`agentPreset` 与接口 `directory` 没有直接关系。
...
- Agent 创建或重启前，初始化流程必须确保用户目录 `{runtime.workdir}/{userId}`、用户默认 `AGENTS.md`、用户默认 `.opencode/`、OpenCode 用户级持久化目录 `.runtime/opencode/config` 与 `.runtime/opencode/share`、所有已声明 preset 的用户工作目录 `{runtime.workdir}/{userId}/{preset}/` 均已存在。
- Agent 创建或重启前，初始化流程必须确保所有已声明 preset 的预设配置源路径 `{runtime.agentPresets.<preset>}/AGENTS.md` 与 `{runtime.agentPresets.<preset>}/.opencode/` 均已存在。
- 将 `{runtime.workdir}/{userId}` 挂载到 Agent 容器内固定工作目录 `/app`；用户默认项目规则和 OpenCode 配置通过根挂载自然成为 `/app/AGENTS.md` 与 `/app/.opencode`。
- 将 `{runtime.workdir}/{userId}/.runtime/opencode/config` 和 `{runtime.workdir}/{userId}/.runtime/opencode/share` 分别挂载到 `/root/.config/opencode` 与 `/root/.local/share/opencode`。
- 根据 `runtime.agentPresets` 声明的全部预设 preset，将每个 `{runtime.agentPresets.<preset>}` 作为只读源目录挂载到 `/agent-preset-config/{preset}`，再由 initContainer 复制到用户侧 `/app/{preset}`。
...
`agent-master` 约定 Kubernetes 挂载关系，并按 OpenCode 官方接口透明代理会话创建请求。`directory` 是 OpenCode 官方 query 参数，由调用方显式传入；`agentPreset` 与接口 `directory` 没有直接关系，`agent-master` 不维护 `agentPreset -> directory` 映射，也不从请求体读取预设名称后拼接目录。
...
- `agent-master` 只透明透传 `directory`，不校验业务预设集合，不根据 `agentPreset` 推导目录。
- 请求体中的预设名称没有 `agent-master` 语义；代理层不得因 agent preset 修改 query 或 body。
...
- `plugins` 只放在用户默认项目级配置 `/app/.opencode` 中，不放在 `runtime.agentPresets` 预设配置包中。
...
- OpenCode 用户级运行配置持久化到 `{runtime.workdir}/{userId}/.runtime/opencode/config` 和 `{runtime.workdir}/{userId}/.runtime/opencode/share`，分别挂载到 `/root/.config/opencode` 与 `/root/.local/share/opencode`。
- `runtime.agentPresets` 约定的 Agent 预设配置包在 Agent 容器启动时通过 `/agent-preset-config/{preset}` 只读源目录提供，再由 initContainer 复制到用户侧 `/app/{preset}/AGENTS.md` 和 `/app/{preset}/.opencode/`。
- 如果 Kubernetes ConfigMap、Secret、volumeMount 或 OpenCode 用户级运行配置发生更新，需要重启 Agent 容器 / Pod，让 OpenCode 进程重新启动后加载新配置。
...
| `running` | Agent 已就绪，可代理 API 请求。 |
| `idle` | Agent 暂无活跃请求，可复用或等待回收。 |
...
NAS 用于承载三类目录：用户 Agent 工作目录、用户默认项目配置、Agent 预设配置包。Agent 实例按用户创建和复用；Kubernetes 启动 Agent 时，按服务配置拼接源路径，并挂载到 Agent 容器内固定路径。
...
│       ├── .runtime/
│       │   └── opencode/
│       │       ├── config/    # 持久化到 /root/.config/opencode
│       │       └── share/     # 持久化到 /root/.local/share/opencode
│       └── {preset}/
│           ├── AGENTS.md      # initContainer 从 agent-presets/{preset}/AGENTS.md 复制
│           ├── .opencode/     # initContainer 从 agent-presets/{preset}/.opencode/ 复制
│           └── ... 用户 preset 工作文件与运行产物，自由组织
└── agent-presets/
    ├── coding/
    │   ├── AGENTS.md
    │   └── .opencode/
...
| `{runtime.workdir}/{userId}/.runtime/opencode/share` | `/root/.local/share/opencode` | OpenCode 用户级数据持久化目录，承接会话、索引、缓存等 OpenCode 自身运行数据。 |
| `{runtime.workdir}/{userId}/{preset}` | `/app/{preset}` | 用户 preset 工作目录；内容由业务自由组织。调用方如需在该目录创建 OpenCode 会话，可显式传入 `directory=/app/{preset}`；`agent-master` 不根据请求体 `preset` 推导该参数。 |
| `{runtime.agentPresets.<preset>}` | `/agent-preset-config/{preset}` | Agent 预设配置包源目录，只读挂载给 initContainer；initContainer 将 `AGENTS.md` 和 `.opencode/` 复制到用户侧 `/app/{preset}`。 |
...
- OpenCode 用户级运行配置按用户持久化：`{runtime.workdir}/{userId}/.runtime/opencode/config -> /root/.config/opencode`，`{runtime.workdir}/{userId}/.runtime/opencode/share -> /root/.local/share/opencode`。
- 用户 preset 工作目录位于 `{runtime.workdir}/{userId}/{preset}`，容器内对应 `/app/{preset}`；目录内容由业务自由组织，`input/`、`output/`、`temp/`、`logs/` 只作为可选示例，不是强制结构。
- `runtime.agentPresets` 是Agent 预设配置包映射；每个 `{runtime.agentPresets.<preset>}` 是 preset 配置根目录，必须包含 `AGENTS.md` 和 `.opencode/`。
- Agent 创建或重启前，必须先初始化所有挂载依赖路径：`{runtime.workdir}/{userId}`、`{runtime.workdir}/{userId}/AGENTS.md`、`{runtime.workdir}/{userId}/.opencode/`、`{runtime.workdir}/{userId}/.runtime/opencode/config`、`{runtime.workdir}/{userId}/.runtime/opencode/share`、所有已声明 preset 的用户工作目录 `{runtime.workdir}/{userId}/{preset}/`，以及所有已声明 preset 的预设配置源路径 `{runtime.agentPresets.<preset>}/AGENTS.md` 与 `{runtime.agentPresets.<preset>}/.opencode/`。
- Kubernetes 启动 Agent 时，必须把 `runtime.agentPresets` 中声明的每个 preset 源目录只读挂载到 `/agent-preset-config/{preset}`，由 initContainer 复制 `AGENTS.md` 和 `.opencode/` 到用户侧 `/app/{preset}`。
...
- `/app/{preset}/AGENTS.md` 和 `/app/{preset}/.opencode` 是 initContainer 从 `/agent-preset-config/{preset}` 复制得到的用户侧文件，不是运行容器的直接只读挂载；避免遮蔽用户 preset 工作目录中的业务文件和运行产物。
- 用户默认 `.opencode/` 可包含 `plugins/`、`commands/`、`modes/`、`themes/` 等配置；preset `.opencode/` 不配置 `plugins/`、`commands/`、`modes/`，只承接 `agents/`、`skills/`、`tools/` 等约束材料。
- 用户默认项目配置、OpenCode 用户级运行配置和Agent 预设配置包都是 Agent 启动时准备、启动时加载的配置，不是创建会话时动态加载的配置；配置更新后需要重启 Agent 容器 / Pod 才能重新加载。
- 新增 `runtime.agentPresets` 时，同样必须先创建用户侧 preset 工作目录 `{runtime.workdir}/{userId}/{preset}/` 和预设配置源路径 `{runtime.agentPresets.<preset>}/AGENTS.md`、`{runtime.agentPresets.<preset>}/.opencode/`；后续新建或重启的 Agent 按最新配置生成 source volume 并在 initContainer 中复制。
...
- Agent 不存在时返回 404。
- 重启不改变 Agent 归属、NAS 挂载约定、Service 代理入口或 preset 处理规则。
...
代理入口以 Agent 实际暴露的 OpenCode Server API 为准。`agent-runtime` 镜像使用 `opencode web --port 4096 --hostname 0.0.0.0` 启动，OpenAPI 3.1 规范页面为 Agent 内部的 `/doc`。Agent API 代理默认是通用透明代理：服务基于用户归属找到当前 Agent Service，去掉 `/agent` 前缀后保留 HTTP 方法、后续路径、查询参数和请求体转发。`POST /agent/session` 也按 OpenCode 官方接口透明代理；`directory` query 参数由调用方显式传入，`agent-master` 不做 `agentPreset -> directory` 转换。
...
- `agentPreset` 与接口 `directory` 没有直接关系；代理层不得读取请求体 `preset` 并拼接或覆盖 `directory`。
- `directory` 不触发配置动态加载，不改变 Agent 实例归属，不影响 Redis Key、Deployment、Service、镜像选择或调度均衡。

OpenCode 配置加载规则：

- OpenCode 项目级配置和用户级运行配置由 OpenCode 进程启动时加载，不是每次创建会话时动态重新加载。
- 用户默认项目规则位于 `{runtime.workdir}/{userId}/AGENTS.md`，通过 `{runtime.workdir}/{userId} -> /app` 根挂载自然成为 `/app/AGENTS.md`。
- 用户默认项目级 OpenCode 配置位于 `{runtime.workdir}/{userId}/.opencode`，通过根挂载自然成为 `/app/.opencode`。
- OpenCode 用户级运行配置持久化到 `{runtime.workdir}/{userId}/.runtime/opencode/config` 和 `{runtime.workdir}/{userId}/.runtime/opencode/share`，分别挂载到 `/root/.config/opencode` 与 `/root/.local/share/opencode`。
- `runtime.agentPresets` 中声明的所有 Agent 预设配置包都必须在 Agent 启动时通过 `/agent-preset-config/{preset}` 源目录只读挂载提供，并由 initContainer 复制到用户侧 `/app/{preset}/AGENTS.md` 和 `/app/{preset}/.opencode/`；预设配置包内 `.opencode/` 可包含 `agents/`、`skills/`、`tools/` 等约束材料，不配置 `plugins/`、`commands/`、`modes/`。
- 如果 Kubernetes ConfigMap、Secret、volumeMount 或用户级 OpenCode 配置发生更新，需要重启 Agent 容器 / Pod，让 OpenCode 进程重新启动后加载新配置。
...
Kubernetes 资源设计只围绕 Agent 运行单元展开，承接用户目录和Agent 预设配置包的挂载约定，不承接完整平台控制面、审计中心、业务 preset 编排或运行时动态配置切换职责。
...
`deploy.yaml` 是 Agent 启动清单，必须体现 `runtime.workdir`、`runtime.agentPresets`、容器端口、镜像、环境变量、volume、volumeMount、Labels / Selector 和安全约束。
...
| `runtime.timeout` | Agent API 代理超时时间。 |
| `runtime.port` | OpenCode Agent 容器监听端口，当前与 `agent-runtime` 镜像约定为 `4096`，Agent 需监听 `0.0.0.0` 以便 Service 访问。 |
| `runtime.workdir` | 用户 NAS 工作目录根路径；Kubernetes 启动 Agent 时拼接 `{runtime.workdir}/{userId}` 并挂载到容器内 `/app`。用户默认项目规则和 OpenCode 配置直接位于 `{runtime.workdir}/{userId}/AGENTS.md` 与 `{runtime.workdir}/{userId}/.opencode/`，通过根挂载自然成为 `/app/AGENTS.md` 与 `/app/.opencode/`；OpenCode 用户级目录通过 `.runtime/opencode/config` 与 `.runtime/opencode/share` 持久化。 |
| `runtime.agentPresets` | Agent 预设配置包映射；每个条目的值是 Agent 预设配置包根目录，目录内包含 `AGENTS.md` 和 `.opencode/`；Kubernetes 启动 Agent 时挂载全部已声明 preset 到 `/agent-preset-config/{preset}`，由 initContainer 复制到用户侧 `/app/{preset}`，用户 preset 工作目录 `/app/{preset}` 保持承载工作文件和运行产物。 |
| `nas.path` | NAS 挂载基础路径，用于说明和组织 users、presets 等目录。 |
...
| `{runtime.workdir}/{userId}/.runtime/opencode/share` | `/root/.local/share/opencode` | OpenCode 用户级数据持久化目录。 |
| `{runtime.agentPresets.<preset>}` | `/agent-preset-config/{preset}` | Agent 预设配置包源目录；initContainer 从中复制 `AGENTS.md` 与 `.opencode/` 到用户侧 `/app/{preset}`。 |
...
- OpenCode 用户级配置与数据目录必须分别持久化到 `{runtime.workdir}/{userId}/.runtime/opencode/config` 和 `{runtime.workdir}/{userId}/.runtime/opencode/share`，再挂载到 `/root/.config/opencode` 与 `/root/.local/share/opencode`。
- `runtime.agentPresets` 中声明的所有预设 preset 都渲染为 `/agent-preset-config/{preset}` 只读 source volume，供 initContainer 复制到用户侧 `/app/{preset}/AGENTS.md` 和 `/app/{preset}/.opencode/`。
- 每个已声明 preset 同时具备两类路径：用户侧工作目录 `{runtime.workdir}/{userId}/{preset}/`，以及预设配置源路径 `{runtime.agentPresets.<preset>}/AGENTS.md`、`{runtime.agentPresets.<preset>}/.opencode/`。
- `runtime.agentPresets.<preset>` 源目录包含 `AGENTS.md` 和 `.opencode/`；渲染时将整个源目录挂载到 `/agent-preset-config/{preset}`，再由 initContainer 复制到 `/app/{preset}`。
- 仅为 `runtime.agentPresets` 中声明的 preset 生成 source volume 和复制命令。
- 使用 source volume 或复制文件时，源文件、源目录和目标父目录必须由初始化流程预先创建，避免 Kubernetes 因挂载目标不存在或复制失败而启动失败。
- 新增 `runtime.agentPresets` 时，必须先创建用户侧 preset 工作目录和预设配置源路径；后续新建或重启的 Agent 按最新配置生成 volumeMount。
...
这些能力应由上游平台控制面完成。`agent-master` 只读取必要的用户标识、预设 preset 名称、配置路径和调度参数，并按约定创建 Kubernetes 挂载与代理关系。
