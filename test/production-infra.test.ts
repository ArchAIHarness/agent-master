...
describe("KubernetesRestWorkloadAdapter", () => {
  ...
  test("creates deployment and service manifests through Kubernetes REST API", async () => {
    ...
    expect(initContainer.volumeMounts).toEqual(runtimeContainer.volumeMounts);
    ...
    expect(JSON.stringify(deployment)).not.toContain("hostPath");
    expect(JSON.stringify(deployment)).not.toContain("/app/.runtime/opencode");
  ...
  test("exposes configured AionUi webui port through runtime Service", async () => {
    const http = new RecordingKubernetesHttpClient();
    const adapter = new KubernetesRestWorkloadAdapter({ http, webuiPort: 3000 });

    await adapter.createService({ image: "agent-image-webui:local", runtime });

    const service = http.requests[0]?.body as Record<string, any>;
    expect(service.spec.ports).toEqual(
      expect.arrayContaining([
        { name: "http", port: 4096, targetPort: 4096 },
