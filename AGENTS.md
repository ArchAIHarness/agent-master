...
- Agent 归属以用户为基本单位。
- NAS 通过 PV/PVC 按用户隔离，整个用户目录是一个 PV，分两个顶层 subPath 挂载：
  - `{runtime.workdir}/{userId}/runtime` → `/app`（用户工作目录 + `.opencode/` + `AGENTS.md`）
  - `{runtime.workdir}/{userId}/global` → `~`（OpenCode 全局配置/data/cache 对应用户主目录下 `.config/`、`.local/share/`、`.cache/`）
