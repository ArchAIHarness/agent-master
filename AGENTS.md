...
`agent-master` 是 Agent 控制面服务，负责单 Kubernetes 集群下 OpenCode Agent 工作负载的调度启停，并代理转发对应 Agent API 请求。

AI 在本仓库内工作时，必须先读取 `README.md` 和本文件，再修改代码或文档。

## 2. 必须做什么

- 保持服务聚焦在 Agent 管理、Kubernetes 单集群调度控制、Agent API 代理。
- 新增功能前先写或更新测试。
...
- 调度逻辑必须显式处理 cluster、namespace、资源名称和错误返回。
- Agent 实例映射、启停状态、租约续约、TTL 回收必须使用 Redis。
- Redis 只做启动后用户 Agent 运行态状态管理；不存配置、会话、消息、文件、凭证、kubeconfig、Token。
- Agent 归属以用户为基本单位。
- NAS 通过 PVC + subPath 按用户隔离，Runtime Deployment 禁止使用 hostPath；必须使用 `runtime.workspacePvcClaimName` 指定的单个 PVC，并以 `runtime.workspacePvcSubPathRoot` 作为 subPath 根目录，分两个顶层 subPath 挂载：
...
- Runtime Deployment 存储卷只允许 `persistentVolumeClaim`，禁止 `hostPath`、本地节点路径或由客户端传入的任意挂载路径。
...
- Agent 实例必须按用户挂载 NAS 工作目录，Runtime Deployment 禁止 hostPath，路径规则为：PVC `{runtime.workspacePvcClaimName}` + subPath `{runtime.workspacePvcSubPathRoot}/{userId}/runtime -> /app`，`{runtime.workspacePvcSubPathRoot}/{userId}/global -> ~`。
