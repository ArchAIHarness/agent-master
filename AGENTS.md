...
`agent-master` 是 Agent Runtime 控制面服务，核心职责是多 Kubernetes 集群下的 Agent Runtime 调度启停、用户归属、租约维护、代理转发和 WebUI 交互 BFF。OpenCode 只是当前接入的 AgentApp 实现，不是服务边界本身。
...
- 当前 `agent-runtime` 镜像内的 OpenCode 实现监听端口约定为 `4096`，Runtime 必须监听 `0.0.0.0` 以便 Service 访问；后续接入其它 AgentApp 时，端口和健康检查必须通过适配器/配置表达。
...
- Agent API 代理必须支持普通 HTTP 与 Runtime 原生 SSE；Runtime 原生 SSE 仍走 `/api/v1/runtime/agent/*` 通用代理入口，平台控制面事件使用独立的 `GET /api/v1/runtime/events`。
- Agent API 默认按通用透明代理转发到 Runtime Service：去掉 `/api/v1/runtime/agent` 前缀，保留后续路径、查询参数、HTTP 方法和请求体。
- 当前唯一透明代理语义特例是会话创建时的 `scene` 转换：读取请求体扩展参数 `scene`，校验后转换为当前 AgentApp 需要的工作目录参数，并从转发请求体中移除 `scene`。
- Agent API 代理路径必须以当前 Runtime 暴露的 OpenAPI 规范或官方文档为准，不得自行编造 AgentApp 原生 API 路径。
...
- Kubernetes 启动 Runtime 时必须挂载 `runtime.scenes` 中声明的所有预设 scene 源目录到 initContainer 只读路径 `/scene-config/{scene}`，由 initContainer 将 `{runtime.scenes.<scene>}/AGENTS.md` 复制到 `/app/{scene}/AGENTS.md`，并将 `{runtime.scenes.<scene>}/.opencode/` 复制到 `/app/{scene}/.opencode/`；主 Runtime 容器只挂载用户工作目录 `{runtime.workdir}/{userId} -> /app`，不再额外挂载 scene 配置源。
- Runtime 创建或重启前，初始化流程必须确保所有复制依赖路径存在：用户根目录 `{runtime.workdir}/{userId}`、用户默认 `AGENTS.md`、用户默认 `.opencode/`、所有已声明 scene 的用户工作目录 `{runtime.workdir}/{userId}/{scene}/`，以及所有已声明 scene 的预设配置源路径 `{runtime.scenes.<scene>}/AGENTS.md` 与 `{runtime.scenes.<scene>}/.opencode/`。
- 新增 `runtime.scenes` 时，必须先创建用户侧 scene 工作目录 `{runtime.workdir}/{userId}/{scene}/` 和预设配置源路径 `{runtime.scenes.<scene>}/AGENTS.md`、`{runtime.scenes.<scene>}/.opencode/`；后续新建或重启的 Runtime 由 initContainer 按最新配置复制到用户 scene 工作目录。
- `runtime.scenes` 场景配置不配置 `plugins/`、`commands/`、`modes/`；`plugins/` 只放在用户默认项目级配置 `/app/.opencode` 中。
- `runtime.scenes` 的运行时目标固定为 `/app/{scene}/AGENTS.md` 和 `/app/{scene}/.opencode`；这两个路径由 initContainer 在 Runtime 创建或重启时覆盖生成，用户 scene 工作目录 `/app/{scene}` 的其它路径保持承载工作文件和运行产物。
- 用户不要手动维护 `{runtime.workdir}/{userId}/{scene}/AGENTS.md` 或 `{runtime.workdir}/{userId}/{scene}/.opencode` 作为长期配置源；它们是由 `runtime.scenes` 源目录复制生成的运行时快照。
- `runtime.scenes.<scene>` 源目录包含 `AGENTS.md` 和 `.opencode/`；渲染时将该源目录挂载到 initContainer 的 `/scene-config/{scene}`，并复制生成 `/app/{scene}/AGENTS.md` 与 `/app/{scene}/.opencode/`。
...
- Runtime 启停与 scene 无关；scene 是 `agent-master` 的会话级扩展参数，用于解析当前会话工作目录。
- 当前 OpenCode AgentApp 创建会话时可以读取请求体 `scene`，但只能用于校验预设集合并拼接容器内目录 `/app/{scene}`；代理转发时必须转换为 OpenCode 当前实现需要的 `directory=/app/{scene}`，且不得向 Runtime 透传 `scene` 字段。
- 当前 OpenCode 项目级配置由 OpenCode 进程启动时加载；如果 `{runtime.workdir}/{userId}/AGENTS.md`、`{runtime.workdir}/{userId}/.opencode` 或 `runtime.scenes` 源目录更新，必须重启 Runtime 容器 / Pod，由 initContainer 重新复制后才能生效。
...
- 不能在 Interaction BFF 的公开请求、响应、错误码或文档中暴露内部 AgentApp 选择参数；模型服务商/模型是用户可选配置，AgentApp 是系统内部装配。
- 不能把附件上传声明为可用却只保存内存元数据；如果声明支持附件，必须保证 Runtime 可读取到对应文件或明确返回不支持。
- 不能绕过测试声称功能完成。
