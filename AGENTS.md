...
- Agent API 默认按通用透明代理转发到 Agent Service：去掉 `/agent` 前缀，保留后续路径、查询参数、HTTP 方法和请求体。
- 当前唯一 API 语义特例是 `POST /agent/session`：读取请求体扩展参数 `scene`，校验后转换为 OpenCode 官方 `directory=/app/{scene}` query 参数，并从转发请求体中移除 `scene`。
...
- Agent Redis Key 固定为 `agent-runtime:user:{userId}`，Agent 状态不得把 scene 作为实例归属条件。
- Agent 实例归属以用户为基本单位。
...
- Kubernetes 启动 Agent 时必须把 `runtime.scenes` 中声明的 scene 源目录只读挂载到 `/scene-config/{scene}`，由 initContainer 复制 `AGENTS.md` 和 `.opencode/` 到用户侧 `/app/{scene}`，避免运行容器直接用只读挂载遮蔽用户 scene 工作目录。
- Agent 创建或重启前，初始化流程必须确保所有挂载依赖路径存在：用户根目录 `{runtime.workdir}/{userId}`、用户默认 `AGENTS.md`、用户默认 `.opencode/`、OpenCode 用户级持久化目录 `.runtime/opencode/config` 与 `.runtime/opencode/share`、所有已声明 scene 的用户工作目录 `{runtime.workdir}/{userId}/{scene}/`，以及所有已声明 scene 的预设配置源路径 `{runtime.scenes.<scene>}/AGENTS.md` 与 `{runtime.scenes.<scene>}/.opencode/`。
- 新增 `runtime.scenes` 时，必须先创建用户侧 scene 工作目录 `{runtime.workdir}/{userId}/{scene}/` 和预设配置源路径 `{runtime.scenes.<scene>}/AGENTS.md`、`{runtime.scenes.<scene>}/.opencode/`；后续新建或重启的 Agent 按最新配置生成 source volume 并在 initContainer 中复制。
- `runtime.scenes` 场景配置不配置 `plugins/`、`commands/`、`modes/`；`plugins/` 只放在用户默认项目级配置 `/app/.opencode` 中。
- `runtime.scenes` 的容器内源路径固定为 `/scene-config/{scene}`；复制目标固定为 `/app/{scene}/AGENTS.md` 和 `/app/{scene}/.opencode`，用户场景工作目录 `/app/{scene}` 保持承载工作文件和运行产物。
- `/app/{scene}/AGENTS.md` 和 `/app/{scene}/.opencode` 是 initContainer 从 `/scene-config/{scene}` 复制得到的用户侧文件，不是运行容器的直接只读挂载。
...
- Agent 启停与 scene 无关；scene 是 `agent-master` 的会话级扩展参数，用于拼接 OpenCode 会话目录。
...
- OpenCode 项目级配置和用户级运行配置由 OpenCode 进程启动时加载；如果 `{runtime.workdir}/{userId}/AGENTS.md`、`{runtime.workdir}/{userId}/.opencode`、`{runtime.workdir}/{userId}/.runtime/opencode/config`、`{runtime.workdir}/{userId}/.runtime/opencode/share` 或 `runtime.scenes` 源配置更新，必须重启 Agent 容器 / Pod 才能生效。
...
- 架构演进优先通过扩展 Kubernetes 集群和 Namespace 分摊 Agent 工作负载，不改变已确认的用户 Agent、NAS 挂载、scene 转换和 Agent API 代理逻辑。
