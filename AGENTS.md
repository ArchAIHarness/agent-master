...
- Agent 归属以用户为基本单位。
- NAS 通过 PV/PVC + `subPath: users/{userId}` 按用户隔离：
  - `{runtime.workdir}/{userId}` → `/app`（用户工作目录 + `.opencode/` + `AGENTS.md`）
  - `{runtime.workdir}/{userId}/.runtime/opencode/config` → `/root/.config/opencode`（OpenCode 用户级配置）
  - `{runtime.workdir}/{userId}/.runtime/opencode/data` → `/root/.local/share/opencode`（OpenCode 用户级数据，含 `auth.json`、会话）
  - `{runtime.workdir}/{userId}/.runtime/opencode/cache` → `/root/.cache/opencode`（provider 包和插件缓存，避免每次重启重新下载）
- **首次创建 Agent（新用户）必须执行完整初始化**：创建 6 个目录 + 写入默认 `AGENTS.md` + 写入默认 `opencode.json` + 设置权限；初始化必须在创建 Deployment 之前完成。
...
- 平台场景化能力通过 OpenCode 原生 `plugin` 机制承载：用户在 `/app/.opencode/opencode.json` 的 `plugin` 字段声明插件包名，OpenCode 启动时下载并缓存到 `/root/.cache/opencode`；master 不持有也不挂载插件源目录。
