...
- Agent 归属以用户为基本单位。
- Runtime 用户存储通过 `runtime.workspacePvcClaimName` 指定的 PVC 按用户隔离；`runtime.workdir` 仅用于 agent-master 侧初始化物理目录，不作为 Runtime Deployment 挂载来源：
  - `{runtime.workspacePvcSubPathRoot}/{userId}/runtime` → `/app`（用户工作目录 + `.opencode/` + `AGENTS.md`）
  - `{runtime.workspacePvcSubPathRoot}/{userId}/global` → `/root` / `~`（OpenCode 全局配置/data/cache 对应用户主目录下 `.config/`、`.local/share/`、`.cache/`）
...
- Runtime Deployment 禁止使用 `hostPath`；用户存储必须通过 `runtime.workspacePvcClaimName` 指定的 PVC 暴露为单个 `persistentVolumeClaim` volume，再用 `runtime.workspacePvcSubPathRoot/{userId}/runtime` 和 `runtime.workspacePvcSubPathRoot/{userId}/global` 两个 subPath 挂载到 `/app` 与 `/root`。
...
- Agent 实例必须按用户挂载 Runtime PVC，路径规则为：`{runtime.workspacePvcSubPathRoot}/{userId}/runtime -> /app` (subPath)，`{runtime.workspacePvcSubPathRoot}/{userId}/global -> /root` (subPath)；`runtime.workdir` 仅用于 agent-master 侧初始化同一用户目录。
