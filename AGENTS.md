...
- Agent 归属以用户为基本单位。
- Runtime Deployment 存储必须使用 PVC + subPath，禁止使用 `hostPath`；测试必须断言 Deployment manifest 不包含 `hostPath`。
- NAS 通过 PV/PVC 按用户隔离，整个用户目录使用一个 PVC，分两个顶层 subPath 挂载：
  ...
  - `runtime.workspacePvcClaimName` 指向 Runtime Pod 使用的 PVC；`runtime.workdir` 仅用于 master 侧初始化同一套存储根目录。
...
- Agent 实例必须按用户挂载 PVC 工作目录，路径规则为：`{runtime.workspacePvcSubPathRoot}/{userId}/runtime -> /app` (subPath)，`{runtime.workspacePvcSubPathRoot}/{userId}/global -> ~` (subPath)，不得在 Runtime Deployment 中渲染 `hostPath`。
